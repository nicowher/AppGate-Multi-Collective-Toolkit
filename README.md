...
**Reports:** `WRITE_RUN_REPORT = True` writes timestamped `reports/run-*.json` / `dryrun-*.json` / `walk-*.json` / `acas-*.json` (no passwords/tokens; file mode `0600` on Unix). Full JSON is printed when `DEBUG = True`. ESXi USM keys print when `PRINT_ESXI_KEYS = True`.
...
- `DEBUG` is **False**. Set `True` for step traces and console JSON dumps.
...
- Lab defaults (`TLS_VERIFY=False`, `SSH_STRICT_HOST_KEY=False`). `DEBUG=False`. Production: TLS and host-key True.
